...
parse_bt_frame:
            {
                ...
                // BT steer_pct → 直接速率模式 (推杆右(CW)→正速率, 松杆→heading hold)
                if (f.steer_pct != 0) {
                    g_balance.bt_yaw_rate_cmd = (float)f.steer_pct * BT_YAW_RATE_MAX / 1000.0f;
                    g_balance.yaw_mode = 1;
                } else {
                    if (g_balance.yaw_mode == 1) {
                        g_balance.target_yaw_angle = g_balance.yaw_angle;  // 松杆时冻结当前朝向
                    }
